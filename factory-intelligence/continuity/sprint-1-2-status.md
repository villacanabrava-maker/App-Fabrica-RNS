...
## Fiscal R2 (rodada `fiscal_1c08890857e77b48c69b6a98`, sha `4f51325`) — 1 achado novo confirmado, resto já triado

Revisão do fiscal OpenAI sem acesso live (só diff inline), 7 pontos. Validados contra o SHA atual (igual ao `base_sha` do parecer) antes de corrigir:

1. **[BLOQUEADOR, fora de escopo] `Vercel – fabricarns` em falha.** Mesmo ponto do achado 4 do Fiscal R1: projeto Vercel do Agent Bridge, infraestrutura separada de `fabricarns-app` (Control Plane desta PR). Nada a corrigir aqui.
2. **[ALTO, já rastreado] Ações administrativas não geram `audit_event`.** Mesmo gap já documentado desde a implementação original ("Dois gaps novos...", item 1) e na tabela de Pendente. Sem mudança de código.
3. **[ALTO, não é código] `validate` (Intelligence CI) skipped.** Esperado por design: `scripts/intelligence/validate-schemas.ts` ainda não existe (entregável de Fase 0, ver `.github/workflows/intelligence-ci.yml` — o próprio job explica isso no `::notice::`). Mesmo ponto do achado 4 do Fiscal R1.
4. **[MÉDIO, confirmado] Deadlock possível no trigger de último owner.** O Fiscal R1 (achado 2) já tinha identificado a corrida e a correção anterior (`FOR UPDATE` na checagem) resolvia a perda de integridade, mas deixava aberto um deadlock real: duas transações removendo/rebaixando owners distintos da mesma organização já travam sua própria linha (OLD) antes do BEFORE ROW trigger rodar, então o `FOR UPDATE` de cada uma espera a linha que a outra já travou — ciclo de espera, Postgres aborta uma das duas. Aceitável para integridade (nunca as duas commitam), mas evitável como falha operacional. Corrigido com gatilho `BEFORE STATEMENT` (`memberships_serialize_owner_changes`) que toma um advisory lock (`pg_advisory_xact_lock`) antes de qualquer linha ser travada pelo UPDATE/DELETE em si — elimina o ciclo de espera (a segunda transação bloqueia no advisory lock antes de travar sua própria linha). `FOR UPDATE` mantido como defesa em profundidade, comentário corrigido para não reivindicar sozinho a serialização. Lock é global (não por organização) — suficiente para o volume de MVP, documentado como tal (`ponytail:` no código).
5. **[MÉDIO, já rastreado] Cobertura de teste insuficiente para OAuth/PKCE, login/signup/reset, criação+seleção de organização, `proxy`, auditoria transacional.** Já documentado na tabela "Pendente" ("Testes de componente (interaction tests), E2E Playwright... Próximo round + Sprint 1.13") — não é achado novo, é escopo deliberadamente adiado, não esta rodada de correção pontual.
